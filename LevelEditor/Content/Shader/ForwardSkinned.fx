...
const float3 groundColor = float3(0.0f, 0.1f, 0.0f);
float3 upVector;
...
struct VertexShaderInput
{
	...
	float4 vertexBoneWeights : BLENDWEIGHT0;
	uint4 vertexBoneIDs : BLENDINDICES0;
};
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
	...
	boneTransform += bonesTransformation[input.vertexBoneIDs.w] * input.vertexBoneWeights.w;
	
	output.vertexPosition = mul(mul(mul(float4(input.vertexPosition, 1.0f), boneTransform), modelMatrix), viewMatrix);
